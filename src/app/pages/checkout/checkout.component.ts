...
export class CheckoutComponent {
  ...
  paypalConfig() {
    this.payPalConfig = {
      currency: "USD",
      clientId:
        "AZ3-HZ_XfEPn1ICOBkV8bhYfR81EckdSGMIfdTqO0D30JWMzuK4pAPvPXY6wwX0jYKAsUXlvYm0dnYcI",
  ...
  onSubmitstatus() {
    let data = {
      ...
      product_id:
        this.apiData.state?.product?.id || this.apiData.state?.productId,
      qty: 1,
    ...
    if (Object.keys(this.paymentStatus).length) {
      ...
      this.checkoutSrv.createOrder(data).subscribe(
        (res: any) => {
          if (res) {
